...
async function viewOrderDetails(orderId, alertId = null) {
    ...
    if (alertId) {
        // For dynamic/local alerts, handle locally
        if (alertId.startsWith('dyn-pending-') || alertId.startsWith('dyn-diag-') || alertId.startsWith('dyn-pickup-') || alertId.startsWith('dyn-delivery-')) {
            ...
            // ✅ Only update Supabase if alertId is a number (integer) – not undefined/string
    ...
    // Apply single order filter & open modal...
    ...
    // Setup modal (rest of the function remains the same)...
    // ... (the existing modal code from your main.js)
